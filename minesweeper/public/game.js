...
$( document ).ready(function() {
  ...
  for(var a = 0; a < height; a++){
    ...
    for(var b =0; b < width; b++){
      if(board[a][b] !== ""){
        var square = $(`<div id="${a}:${b}" class="square" bomb=${board[a][b]} data-checked = false>B</div>`)
      }else{
        var square = $(`<div id="${a}:${b}" class="square" bomb= "A" data-checked = false></div>`)
      }
...
$(".outerBoard").click(function(e){
  ...
  if(test1 === "B"){
    ...
    console.log('this is e.target', $(e.target).data( "checked" ));
    var cords = e.target.id;
    var row = cords[0];
    var column = cords[2];
    console.log(row, column, $("#" + row + "\\:" + column)[0]);
    $(e.target).data( "checked", true);
    console.log('changed', $(e.target).data( "checked"));
    console.log(row, column, $("#" + row + "\\:" + column)[0]);
    que.push([row, column])
  }
});

// function search(cords){
//   if(isBomb(cords) === "B"){
//     return false;
//   }else{
//     return true;
